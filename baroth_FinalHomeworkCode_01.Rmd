...
- Super cool prettification! Love the theme! We'll take inspiration.
  + **Response: Thank you!**

- Challenge 1, Step 3: You're getting a warning sign about 'stringr' being built under version 4.2.3. Perhaps there is a way to eliminate that.
  + **Response: I made the warning not show up in the html output.**

- For Challenge 2, Step 4, I think the question asks us to extract elements from column 2, row 2 to column 6, row 9 rather than just those two elements. This may have just been a misunderstanding on your part.
  + **Response: Aha. I did misunderstand the direction. I have edited that challenge to show element [2,2] through [6,9].**

- For Challenge 3, perhaps there is a way for the output to show only a few lines instead of the entire array. This would help with visual clarity of the knit file.
  + **Response: I restricted the print output to only the first two matrix slices.**

- I really liked how you explained your thought process using the "Direction" subheadings and the walkthrough at the end. Really great work!
  + **Response: Thanks!**
  
**Additional notes on my responses to peer commentary are included below in bold.**
...
```{r Challenge 1 load stringr, warning=FALSE}
library(stringr) #load stringr in the library
```
**Loading the stringr function gives a warning. I have hidden the warning using the argument warning=FALSE in the chunk setup.**

Nxxt, use str_split_1() to divide the string. The second argument " " has a space between the quotation marks to indicate that the string should be divided everywhere there is a space. You could tell it to divde using other markers if wanted. The benefit of str_split_1() as opposed to str_split is that it produces a character vector and not a list which makes the next step easier.
...
To extract a range of elements, use bracket notation m[2,2] in combination with the range function 2:6 to get all values between rows 2 and 6 and columns 6 and 9.
```{r Challenge 2 step 4}
x <- m[2:6,2:9] #extract and assign the elements to the variable x
...
```{r Challenge 3 step 1, R.options=list(max.print=50)}
a <- array(data = 400:1, dim = c(5, 5, 4, 4)) #create the array
a #check that the array is correct
```
**I have restricted the print output of the array to only two matrix slices for clarity. I did this using R.options=list(max.print=50) in the chunk setup.**
...
1. I learned that I can control what is visible in the evaluation sections of the HTML output such as hiding a warning or limiting the number of lines shown.
2. I learned how to make the knitted HTML from an R markdown more readable with section headers and more aesthetically pleasing with different themes and text styles.
3. I learned how to correct various errors that might come up that prevent an R markdown file from knitting such as missing a package or having chunks with the same name, etc.

# Two things I liked in my peers' work

1. I thought that Jess's second version of how to make a hierarchical list by creating the $variables during the initial creation of the list was cool and potentially less confusing while writing the code.
2. I liked that Bhavya included and image with a caption, and it is nice to have another example of how to do so.